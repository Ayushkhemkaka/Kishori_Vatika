...
export default async function MarketingHomePage() {
  ...
  return (
    <div className="space-y-12">
      ...
      <section className="space-y-6">
        ...
        <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          {roomCards.map((room, index) => (
            <article
              key={room.slug}
              className="group flex flex-col justify-between overflow-hidden rounded-2xl border border-emerald-100 bg-white shadow-md shadow-emerald-100/40 transition hover:border-emerald-200 hover:shadow-emerald-100/70"
            >
              <div>
                ...
                <div className="space-y-3 p-5">
                  <h3 className="font-sans text-lg font-medium leading-snug tracking-[-0.005em] text-stone-900">
                    <Link href={`/rooms/${room.slug}`} className="hover:text-emerald-700">
                      {room.title}
                    </Link>
                  </h3>
              ...
              <div className="flex items-center justify-between px-5 pb-5 text-xs text-stone-500">
                <span>{home.roomsSection.inclusionNote}</span>
                <Link
                  href={`/rooms/${room.slug}`}
                  className="font-medium text-emerald-700 group-hover:text-emerald-600"
                >
      ...
      <section className="kv-themed-section grid gap-8 rounded-3xl border border-emerald-100 bg-white p-6 sm:p-8 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)] lg:items-center">
        {/* Left: where we are. Right: what the stay is like. */}
        ...
        <div className="space-y-4">
          ...
          >
            {home.experiencesSection.contactCta.label} &rarr;
          </Link>
        </div>
      </section>

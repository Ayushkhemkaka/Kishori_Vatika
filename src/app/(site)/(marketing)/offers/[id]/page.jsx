...
async function OfferDetailPage({
  params
}) {
  const { id } = await params;
  const offer = await getOfferById(id);
  ...
  const includes = offer.features.length > 0 ? offer.features.map((f) => f.value ? `${f.label}: ${f.value}` : f.label) : [offer.description];
    ...
    <section className="grid gap-8 lg:grid-cols-[minmax(0,1.15fr)_minmax(0,0.85fr)]">
      <div className="space-y-4 rounded-2xl border border-emerald-100 bg-white p-5 shadow-md shadow-emerald-100/40">
        ...
        <ul className="mt-2 space-y-2 text-sm text-stone-600">
          {includes.map((item) => <li key={item} className="flex gap-2">

...
function EnquiryPageContent() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        const msg = details ? Object.entries(details).map(([k, v]) => `${k}: ${Array.isArray(v) ? v[0] : v}`).join("; ") : data?.error ?? "Something went wrong. Please try again.";
        setErrorMessage(msg);
        return;
      }
  ...
  return <div className="grid gap-10 lg:grid-cols-[minmax(0,1.3fr)_minmax(0,1fr)] lg:items-start">
  ...
  >
              {formState === "submitting" ? "Sending your enquiry..." : formState === "submitted" ? "Enquiry sent" : "Send enquiry"}
            </button>
          ...
          {isSubmitted && <p className="rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm text-emerald-800">

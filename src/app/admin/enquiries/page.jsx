...
async function AdminEnquiriesPage({
  ...
  return <div className="space-y-6">
      ...
      <div className="overflow-x-auto rounded-xl border border-white/10">
        <table className="w-full min-w-[640px] text-left text-sm">
          ...
          <tbody className="divide-y divide-white/5">
            {enquiries.length === 0 ? <tr>
                <td colSpan={6} className="px-4 py-8 text-center text-slate-400">
                  No enquiries yet.
                </td>
              </tr> : enquiries.map((e) => <tr key={e.id} className="bg-slate-950/40">
    ...
    className={`rounded-full border px-4 py-2 text-sm ${hasPrev ? "border-white/20 text-slate-200 hover:bg-white/5" : "pointer-events-none border-white/10 text-slate-500"}`}
  >
            Previous
          </Link>
          <Link
    ...
    className={`rounded-full border px-4 py-2 text-sm ${hasNext ? "border-white/20 text-slate-200 hover:bg-white/5" : "pointer-events-none border-white/10 text-slate-500"}`}
  >
            Next
          </Link>
        </div>

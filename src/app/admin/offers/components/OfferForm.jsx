...
function OfferForm({ initial }) {
  ...
  const [features, setFeatures] = useState(
    initial?.features?.length ? initial.features.map((f) => ({ ...f, id: f.id || `f-${Math.random()}` })) : [{ id: "f1", label: "", value: "" }]
  );
  ...
  function updateFeature(id, field, value) {
    setFeatures(
      (prev) => prev.map((f) => f.id === id ? { ...f, [field]: value } : f)
    );
  }
  ...
  async function handleSubmit(e) {
    ...
    try {
      const payload = {
        ...
        features: features.filter((f) => f.label.trim() || f.value.trim()).map((f) => ({ id: f.id.startsWith("f-") ? void 0 : f.id, label: f.label, value: f.value }))
      };
      let offerId;
  ...
  >
        ...
        <ul className="mt-2 space-y-2">
          {features.map((f) => <li key={f.id} className="flex gap-2">
              <input
    value={f.label}
  ...
  />
      ...
      {error && <p className="rounded-lg border border-rose-400/40 bg-rose-900/20 px-3 py-2 text-sm text-rose-50">
          {error}
        </p>}
